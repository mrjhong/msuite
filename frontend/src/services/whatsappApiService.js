...
export const apiScheduleMessageWithFile = async (formData) => {
    const urlApi = process.env.REACT_APP_URL_API;
    const token = localStorage.getItem('token');
    
    try {
        console.log('📤 Programando mensaje con archivo usando FormData');
        ...
        const response = await fetch(`${urlApi}/whatsapp/schedule/messages`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                // No establecer Content-Type, el navegador lo hará automáticamente para FormData
        ...
        if (!response.ok) {
            const errorData = await response.json();
            console.error('❌ Error del servidor:', errorData);
            return { success: false, error: errorData.error || 'Error programando mensaje con archivo' };
        }
        ...
        console.error('❌ Error programando mensaje con archivo:', error);
